...
from krkn_ai.models.config import HealthCheckConfig, HealthCheckApplicationConfig, HealthCheckResult
    ...
    def test_stop_terminates_all_threads(self, mock_get):
        ...
        app_configs = [
            HealthCheckApplicationConfig(
                name=f"app-{i}",
                url=f"http://localhost:808{i}/health",
                interval=1
            )
            for i in range(3)
        ]
...
class TestHealthCheckWatcherResults:
    ...
    def test_summarize_response_time_detects_outliers(self, mock_get):
        ...
        mock_responses = [
            Mock(
                status_code=200,
                elapsed=Mock(total_seconds=lambda: rt)
            )
    ...
    @patch('krkn_ai.chaos_engines.health_check_watcher.requests.get')
    def test_summarize_response_time_returns_zero_with_insufficient_data(self, mock_get):
