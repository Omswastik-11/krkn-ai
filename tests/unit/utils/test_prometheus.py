    ...
    def test_create_prometheus_client_uses_environment_variables_when_set(self):
        ...
        with patch.dict(os.environ, {"PROMETHEUS_URL": "https://prometheus.example.com", "PROMETHEUS_TOKEN": "test-token"}):
            with patch('krkn_ai.utils.prometheus.KrknPrometheus') as mock_prom_class:
                ...
                mock_prom_class.assert_called_once_with("https://prometheus.example.com", "test-token")
                assert client == mock_client

    def test_create_prometheus_client_fetches_url_from_openshift_route_when_env_not_set(self):
            ...
            with patch('krkn_ai.utils.prometheus.is_openshift', return_value=True):
                ...
                with patch('krkn_ai.utils.prometheus.run_shell') as mock_shell:
                    ...
                    with patch('krkn_ai.utils.prometheus.KrknPrometheus') as mock_prom_class:
                        ...
                        assert call_args[0] == "https://thanos-query-openshift-monitoring.apps.example.com"
